...
const SORT_OPTIONS: readonly DropdownOption<string>[] = [
  { value: "updatedAt,desc", label: "수정 최신순" },
  { value: "createdAt,desc", label: "작성 최신순" },
...
export default function PostListPage() {
  ...
  useEffect(() => {
    void load(pageParam, qParam, sizeParam, sortParam);
  ...
  return (
    <div className="boardPage">
      <section className="card cardPad boardHero">
        <div className="boardHeroMain">
          <div className="pageEyebrow">Community Feed</div>
          <h2 className="pageTitle boardTitle">게시글</h2>
          <p className="boardIntro muted">
            제목을 먼저 훑고, 작성자와 시간은 보조 정보로 빠르게 확인할 수 있는 게시판
            목록입니다.
          </p>
        </div>

        <div className="boardHeroMeta">
          <span className="pill">총 {totalCountText}개</span>
          {hasQuery && <span className="pill">검색어 {qParam.trim()}</span>}
          <span className="pill">정렬 {SORT_OPTIONS.find((item) => item.value === sortParam)?.label}</span>
        </div>
      </section>

      <section className="card cardPad boardToolbar">
        <div className="boardToolbarTop">
          <div className="boardSearchGroup">
            <input
              ref={inputRef}
        ...
        <div className="boardToolbarBottom">
      ...
      <div className="boardFeed">
        ...
        {!showLoadingFirst && !showEmpty && (
          <div className="postList">
            {(data?.items ?? []).map((p) => {
              const excerpt = toExcerpt(p.summary);

              return (
                <article
                  key={p.id}
                  className="card cardPad postCard postCardFeed"
                  tabIndex={0}
                  role="link"
                ...
                >
                  <div className="postCardMain">
                    <div className="postMetaRow">
                      <span className="postMetaLabel">#{p.id}</span>
                      {p.authorLoginId && <span className="postMetaLabel">{p.authorLoginId}</span>}
                      <span className="postMetaLabel">{formatWrittenAt(p.createdAt)}</span>
                    </div>
                  ...
                  <div className="postCardSide">
                    ...
                    <div className="postSideTime">{formatWrittenAt(p.updatedAt)}</div>
                  </div>
                </article>
              );
            })}
      ...
      <section className="card cardPad boardPagination">
        <div className="boardPaginationMeta">
          <span className="muted">페이지 이동</span>
